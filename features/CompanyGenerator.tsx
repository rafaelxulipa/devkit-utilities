...
import { generateCompany } from '../utils/documentGenerators';
...
const CompanyGenerator: React.FC = () => {
    ...
    const handleGenerate = useCallback(() => {
        setCompany(generateCompany(isMasked));
    }, [isMasked]);
    ...
    const DataRow: React.FC<{ label: string; value: string }> = ({ label, value }) => (
        <div className="flex items-center justify-between p-3 bg-light-bg dark:bg-dark-bg rounded-md">
            ...
            <div className="flex items-center space-x-2">
                ...
                <button onClick={() => handleCopy(value, label)} className="p-1 rounded text-light-secondary hover:text-light-primary dark:text-dark-secondary dark:hover:text-dark-primary">
                    <CopyIcon />
                </button>
            </div>
        </div>
        ...
        <div className="space-y-6 max-w-lg mx-auto">
            {company && (
                <div className="space-y-3 border border-light-secondary/20 dark:border-dark-secondary/20 rounded-lg p-4">
                    <DataRow label="Nome Fantasia" value={company.nome} />
                    <DataRow label="CNPJ" value={company.cnpj} />
                    <DataRow label="Inscrição Estadual (SP)" value={company.ie} />
                </div>
            )}
