...
// --- MD5 (RFC 1321) ---
// Web Crypto (SubtleCrypto) não implementa MD5, então usamos uma implementação pura em JS
// apenas para fins de checksum/compatibilidade (MD5 não é seguro para uso criptográfico).
...
const binlMD5 = (x: number[], len: number): [number, number, number, number] => {
    x[len >> 5] |= 0x80 << (len % 32);
    x[(((len + 64) >>> 9) << 4) + 14] = len;
...
const bytesToBinl = (bytes: Uint8Array): number[] => {
    const bin: number[] = [];
...
const binlToHex = (binarray: [number, number, number, number]): string => {
...
// --- SHA-1 / SHA-256 / SHA-384 / SHA-512 (Web Crypto / SubtleCrypto) ---
export type ShaAlgorithm = 'SHA-1' | 'SHA-256' | 'SHA-384' | 'SHA-512';

export const hashSHA = async (text: string, algorithm: ShaAlgorithm): Promise<string> => {
